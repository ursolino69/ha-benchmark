# Deployment 0.8.0 · calibrated R4
...
```powershell
scp "$env:USERPROFILE\Downloads\smartdomo-ha-benchmark-v0.8.0.zip" root@217.154.22.176:/root/
...
mkdir -p /root/ha-benchmark-0.8.0
unzip -q -o /root/smartdomo-ha-benchmark-v0.8.0.zip -d /root/ha-benchmark-0.8.0
bash /root/ha-benchmark-0.8.0/smartdomo-ha-benchmark/install-community.sh
```
...
install/update **HA Benchmark 0.8.0**, and restart the app.
...
3. Run Light three times under comparable idle conditions. Verify raw values, category indices and
   the Smartdomo Index in JSON.
4. Publish one R4 entry after reviewing the data preview and verify it in the R4 ranking.
...
7. Switch to the R3 archive and verify that R3 and R4 entries are never shown or compared together.
8. Check desktop/mobile layouts and logs; no alias, IP, raw payload or deletion key should appear.

## Important boundary

Version 0.8.0 freezes R4 calibration `GREEN-CORE-2026-09-D`. The public service keeps R4 and R3
strictly separate by methodology, calibration and profile. Community data remains user-submitted and
